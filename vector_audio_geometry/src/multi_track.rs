...
/// The optimization is intentionally simple and deterministic; it is a robust
/// prototype before replacing it with a more sophisticated spline optimizer.
pub fn refine_paths(
...
mod tests {
    use super::*;
    use crate::phasegrad::C32;

    #[test]
    fn energy_grid_is_finite() {
        let g = vec![
            vec![Grad{coeff:C32::new(1.0,0.0),freq_hz:100.0,time_s:0.0,confidence:1.0}; 4],
            vec![Grad{coeff:C32::new(0.5,0.0),freq_hz:200.0,time_s:0.0,confidence:1.0}; 4],
        ];
        let (e,c,f) = reassigned_energy_grid(&g,20.0,12);
        assert_eq!(c,2);
        assert_eq!(f,4);
